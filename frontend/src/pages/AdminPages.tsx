    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Order Details: ${order.id}`}>
            <div className="space-y-4">
                ...
                <div className="flex justify-end pt-4 space-x-2">
                    <a href={`${import.meta.env.VITE_API_URL}/api/orders/${order.id}/invoice`} target="_blank" rel="noopener noreferrer">
                ...
                <Table headers={['Backup ID', 'Date Created', 'File Size', 'Actions']}>
                    {backups.map(backup => (
                         <tr key={backup.id}>
                            ...
                            <Td>
                                <div className="flex space-x-3">
                                    <a href={`${import.meta.env.VITE_API_URL}/api/backups/${backup.id}/download`} download={backup.fileName}>
